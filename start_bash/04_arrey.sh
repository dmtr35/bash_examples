...
# создать массив
arr=(one two three)
arr+=(four five six)                    # добавить в конец массива
arr[3]="4"
arr[4]="5"
declare -a dec_arr                      # создать массив (array) с именем dec_arr
...
echo "${#a[3]}"                         # длина элемента с индексом 3
...
# Лучший универсальный способ — printf
arr=(5 6 7 3 12)
printf -v result '%s;' "${arr[@]}"
result=${result%;}   # убрать последний ;
echo "$result"                                      # 5;6;7;3;12

# Канонический Bash-способ — через IFS
arr=(5 6 7 3 12)
...
# Через paste (когда данные уже построчно)
printf '%s\n' "${arr[@]}" | paste -sd ';'           # 5;6;7;3;12
